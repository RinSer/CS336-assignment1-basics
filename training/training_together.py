...
def training_loop(
    ...
    val_steps: int = 1000):
    """
    To check stats run `tensorboard --logdir=runs`
    """
    model = TransformerLM(
        vocab_size,
        context_length,
    ...
    val_dataset = np.memmap(val_data_path, mode="r", dtype=np.uint16) \
    ...
    for i in range(iteration, num_iterations):
        ...
        # Validation logging
        if val_dataset is not None and (i + 1) % val_steps == 0 or i == num_iterations - 1:
            model.eval()
            with torch.no_grad():
